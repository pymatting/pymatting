...
@njit("i8(i8, f8[:], i8[:], i8[:], f8[:], i8[:], i8[:], f8, f8, i8)", cache=True)
def _ichol(n, Av, Ar, Ap, Lv, Lr, Lp, discard_threshold, shift, max_nnz):
    ...
    b = np.zeros(n, np.bool8)     # b[i] indicates if the i-th element of column j is non-zero
...
@njit("void(f8[:], i8[:], i8[:], f8[:], i8)", cache=True)
...
def ichol(
    ...
    for shift in shifts:
        ...
        if nnz == -1:
            ...
            print("Thresholded incomplete Cholesky decomposition failed due to insufficient positive-definiteness of matrix A with parameters:")
        ...
        if nnz == -2:
            raise ValueError("Thresholded incomplete Cholesky decomposition failed because more than max_nnz non-zero elements were created. Try increasing max_nnz or discard_threshold.")

    if nnz < 0:
        raise ValueError("Thresholded incomplete Cholesky decomposition failed due to insufficient positive-definiteness of matrix A and diagonal shifts did not help.")
